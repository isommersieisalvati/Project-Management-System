...
export const fetchProducts = createAsyncThunk(
  ...
  async ({ search, sortBy, sortOrder }: { search?: string; sortBy?: string; sortOrder?: string } = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(`${API_URL}/products?${params}`, getAuthConfig())
...
export const updateProduct = createAsyncThunk(
  ...
  async ({ id, formData }: { id: number; formData: FormData }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.put(`${API_URL}/products/${id}`, formData, config)
      return response.data.product
    } catch (error: any) {
...
export const { clearError, setSearchTerm, setSortBy, setSortOrder, setSelectedProduct } = productSlice.actions
